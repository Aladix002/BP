...
ManualTeleopNode::ManualTeleopNode(std::shared_ptr<std::atomic<bool>> manual_mode)
    ...
    // Musi byt vacsi ako key repeat delay (na RPi moze byt 700-1000 ms)
    this->declare_parameter<int>("key_release_timeout_ms", 1100);
    ...
    RCLCPP_INFO(this->get_logger(), "Manual teleop: WASD/sipky -> /manual_motor_cmd | u/i o/p = trim | m = prepinat rezim");
}
...
void ManualTeleopNode::timer_cb() {
    ...
    for (;;) {
        ...
        if (k == KeyDir::Stop) {
            ...
            RCLCPP_INFO(this->get_logger(), "Rezim: %s", new_val ? "MANUAL" : "AUTO (lidar)");
        } else {
    ...
    double L = 0.0, R = 0.0;  // L = lavy motor, R = pravy motor (konzistentne s /motor_cmd a UGV L/R)
    ...
    if (!up_ok && !down_ok && !left_ok && !right_ok) {
        ...
        // Obe sipky naraz alebo konflikt -> zastav
        L = 0;
        R = 0;
    }
