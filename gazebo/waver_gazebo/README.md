...
```bash
ros2 launch waver_gazebo gazebo.launch.py
```

RViz sa spustí **predvolene** spolu s Gazebo (`use_rviz:=true`). Bez grafického výstupu: `use_rviz:=false`.
...
- `use_rviz` _(bool)_: (len `gazebo.launch.py`) Spustí RViz2 s `rviz/gazebo.rviz`.  
  Default: `true`
